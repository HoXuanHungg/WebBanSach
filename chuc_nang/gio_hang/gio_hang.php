<?php 
	if(isset($_GET['id']) and $_SESSION['trang_chi_tiet_gio_hang']=="co")
	{
		...
		if(isset($_SESSION['id_them_vao_gio'])) //kiem tra da ton tai session ch
		{
			$so=count($_SESSION['id_them_vao_gio']); //dem so luong session hien tai( 1ss = 1 sp)
			$trung_lap="khong";
			for($i=0;$i<count($_SESSION['id_them_vao_gio']);$i++)
			{
				...
				{
					$trung_lap="co";
					$vi_tri_trung_lap=$i; //ss mấy trùng lặp
					break;
				}
			}
			if($trung_lap=="khong")
